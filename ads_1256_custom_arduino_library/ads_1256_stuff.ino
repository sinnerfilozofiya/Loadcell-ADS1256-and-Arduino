...
void initADS() {
  ...
  SetRegisterValue(MUX,DIFF_0_1); //set the mux register
...
void read_two_values() {
  ...
  SPI.transfer(WREG | MUX); // send 1st command byte, address of the register
  SPI.transfer(0x00);   // send 2nd command byte, write only one register
  SPI.transfer(DIFF_0_1);     //pins registers 2 and 3
...
void read_four_values() {
  // Variables to store the ADC values for each differential channel
  int32_t adc_val1 = 0;
  int32_t adc_val2 = 0;
  int32_t adc_val3 = 0;
  int32_t adc_val4 = 0;
  ...
  for (int i = 0; i < 4; i++) {
    waitforDRDY(); // Wait until DRDY is LOW
    SPI.beginTransaction(SPISettings(SPI_SPEED, MSBFIRST, SPI_MODE1));
    digitalWriteFast(10, LOW); // Pull SS Low to Enable Communications with ADS1256

    // Change the MUX register to select the desired differential input
    SPI.transfer(WREG | MUX); // Send 1st command byte, address of the MUX register
    SPI.transfer(0x00);       // Send 2nd command byte, write only one register
    SPI.transfer(mux_settings[i]); // Set MUX for the desired channel pair

    // Synchronize with the ADC
    delayMicroseconds(2); // Delay by at least 4x the SPI clock period
    SPI.transfer(SYNC);
    delayMicroseconds(5); // Additional delay to ensure synchronization
    SPI.transfer(WAKEUP);
    delayMicroseconds(1);
    ...
    if (adc_val > 0x7FFFFF) { // If MSB is set (negative number)
      adc_val = adc_val - 16777216; // Convert to signed 32-bit integer
    }

    // Store the read value in the corresponding variable
    switch (i) {
      case 0: adc_val1 = adc_val; break;
      case 1: adc_val2 = adc_val; break;
      case 2: adc_val3 = adc_val; break;
      case 3: adc_val4 = adc_val; break;
    }
